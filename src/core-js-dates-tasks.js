...
function getNextFriday(date) {
  const ans = new Date(
    ((12 - date.getUTCDay()) % 7 ? (12 - date.getUTCDay()) % 7 : 7) *
      1000 *
      60 *
      60 *
      24 +
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return Math.ceil(
    (1 + Date.parse(dateEnd) - Date.parse(dateStart)) / (1000 * 24 * 60 * 60)
  );
}
...
function formatDate(date) {
  return new Intl.DateTimeFormat('en', {
    ...
    .format(Date.parse(date) + new Date(date).getTimezoneOffset() * 60 * 1000)
    .toUpperCase();
}
...
function getWeekNumberByDate(date) {
  ...
  firstDay.setDate(1);
  firstDay.setMonth(0);
  const firstWeek = 7 - ((firstDay.getDay() + 6) % 7);
  const ans = (newDate - Date.parse(firstDay)) / (1000 * 60 * 60 * 24);
  const add = firstWeek < 4 ? 0 : 1;
  if (ans < firstWeek) return 1;
  return Math.ceil((ans - firstWeek + 1) / 7) + add;
}
...
function getNextFridayThe13th(date) {
  while (date.getDate() !== 13 || date.getDay() !== 5) {
    if (date.getDate() < 13) date.setDate(13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const date = new Date();
  const arrS = period.start.split('-');
  date.setUTCDate(arrS[0]);
  date.setUTCMonth(arrS[1] - 1);
  date.setUTCFullYear(arrS[2]);
  const end = new Date(date);
  const arrE = period.end.split('-');
  end.setUTCDate(arrE[0]);
  end.setUTCMonth(arrE[1] - 1);
  end.setUTCFullYear(arrE[2]);
  const sum = countWorkDays + countOffDays;
  let i = 0;
  const ans = [];
  while (Date.parse(date) <= Date.parse(end)) {
    if (i % sum < countWorkDays)
      ans.push(
